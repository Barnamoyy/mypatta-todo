...
import { useTodayTasks, TaskProps } from "@/hooks/useTodayTasks";
...
const TodayTasks = () => {
  ...
  const { todayTasks, loading, refetch } = useTodayTasks();
    ...
    <>{selectedTaskIds.length > 0 && (
      ...
      {todayTasks && todayTasks.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {[...(todayTasks || [])].reverse().map((task: any) => (
            <TaskCard task={task} onOpen={handleOpen} onSelect={handleSelect}
